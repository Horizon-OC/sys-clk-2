...
void BaseMenuGui::preDraw(tsl::gfx::Renderer* renderer) {
    ...
    renderer->drawString(displayStrings[20], false, dataPositions[0], y, SMALL_TEXT_SIZE, tempColors[SysClkThermalSensor_Battery]);  // Battery
...
void BaseMenuGui::refresh()
{
    ...
    sprintf(displayStrings[19], "%u%%", context->partLoad[SysClkPartLoad_CPUMax] / 10);

    millis = context->temps[SysClkThermalSensor_Battery]; // Battery
    sprintf(displayStrings[20], "%u.%u °C", millis / 1000U, (millis % 1000U) / 100U);
    tempColors[SysClkThermalSensor_Battery] = tsl::GradientColor(millis * 0.001f);
    ...
    //sprintf(displayStrings[26], "%u", context->speedos[SysClkSpeedo_CPU]);
